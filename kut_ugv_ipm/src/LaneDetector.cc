...
//     #endif
  ...
  //Get IPM
  CvSize ipmSize = cvSize((int)stopLineConf->ipmWidth, (int)stopLineConf->ipmHeight);
  CvMat * ipm;
  ipm = cvCreateMat(ipmSize.height, ipmSize.width, inImage->type);
  //mcvGetIPM(inImage, ipm, &ipmInfo, cameraInfo);
  ipmInfo.vpPortion = stopLineConf->ipmVpPortion;
 ...
 *
 */
 void mcvInitLaneDetectorConf(const char * fileName,
    LaneDetectorConf *stopLineConf)
{
  //parsed camera data
 ...
 * \param splineScoreJitter Number of pixels to go around the spline to compute score
 ...
 * \param prevSplines the splines from the previous frame, to use as initial seeds
  ...
  for (int i=0; i<numIterations; i++)
  {
    ...
    else
    {
      ...
      curSpline = mcvFitBezierSpline(samplePoints, splineDegree);
       	    //fSHOW_MAT(samplePoints, "Sampled points");
    } // else
...
void mcvDrawSpline(CvMat *image, Spline spline, CvScalar color, int width){
  //get spline pixels
  CvMat *pixels = mcvGetBezierSplinePixels(spline, .05, cvSize(image->width, image->height), false);
  //if no pixels
  if (!pixels)
    return;
